...
@Component({
  ...
  templateUrl: './supplier-shell.component.html',
  styleUrls: ['./supplier-shell.component.css']
})
export class SupplierShellComponent implements OnInit {
  ...
  addSupplier ()
  {
    ...
    this.dataservice.addSupplier(addSupplier).subscribe(rea=>{
      this.resetForm ()
...
editSupplier(
  ...
  return
    ...
    this.dataservice.editSupplier(editSupplier, this.supplierId).subscribe(
  ...
  whenDeletePopupConfirm = (e) => {
    if (e) {
      this.dataservice.deleteSupplier(this.deletedId).subscribe(
        res=>{
          console.log(res)
